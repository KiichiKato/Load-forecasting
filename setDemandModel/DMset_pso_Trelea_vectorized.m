...
function [OUT,varargout]=DMset_pso_Trelea_vectorized(functname,D,varargin)
...
elseif length(mv)==D     
 velmaskmin = repmat(DMset_forcerow(-mv),ps,1); % min vel
 velmaskmax = repmat(DMset_forcerow( mv),ps,1); % max vel
else
...
% format of pos= (particle#, dimension)
 % construct random population positions bounded by VR
  pos(1:ps,1:D) = DMset_normmat(rand([ps,D]),VR',1);
 ...
 % construct initial random velocities between -mv,mv
  vel(1:ps,1:D) = DMset_normmat(rand([ps,D]),...
      [DMset_forcecol(-mv),DMset_forcecol(mv)]',1);
